...
void CanFTP_Session_FileBlock_ResetFramesFlags(CanFTP_Session_FileBlock_t* block)
{
    ...
    // Сброс флагов обработки субблоков
    {
...
void CanFTP_Session_FileBlock_MoveBlockData(CanFTP_Session_FileBlock_t* block, uint8_t* data)
{
    ...
    // Назначение реальной длины блока
    block->length = actualFileLength;
}
/*
    ...
    , uint8_t* data)
{
    ...
    {
        uint32_t i = 0;
        // Перенос данных из кадра
        for (i = 0; i < CANFTP_FILEBLOCK_FRAMESIZE; i++)
        {
            data[i] = block->data[frameIndex * CANFTP_FILEBLOCK_FRAMESIZE + i];
        }
        block->framesFlags[frameIndex] = CANFTP_TRUE;
    }
}
/*
    ...
    , uint8_t* data)
{
    ...
    else
    {
        ...
        for (i = 0; i < CANFTP_FILEBLOCK_FRAMESIZE; i++)
        {
            block->data[frameIndex * CANFTP_FILEBLOCK_FRAMESIZE + i] = data[i];
        }
        block->framesFlags[frameIndex] = CANFTP_TRUE;
    }
}
...
CanFTP_Logical_t CanFTP_Session_FileBlock_VerifySubblocks(CanFTP_Session_FileBlock_t* block)
{
    ...
    // Сброс параметров субблоков
    for (i = 0; i < subblocksCount; i++)
    {
        block->subblocksFlags[i] = CANFTP_TRUE;
    }
...
{
    uint32_t i = 0;
    // Сброс значений контрольной суммы
    {
    ...
    {
        uint32_t* crcLow = (uint32_t*)&crcArray[0];
        uint32_t* crcHigh = (uint32_t*)&crcArray[4];

        uint32_t crcLowPrevious = *(crcLow);
        // Проход по содержимому блока
        for (i = 0; i < block->length; i++)
        {
            uint8_t dataElement = block->data[i];

            if ((*(crcLow) & 0x80000000) == 0x80000000)
            {
                *(crcHigh) = (*(crcHigh) << 1) | 0x01;
                *(crcLow) = *(crcLow) << 1;
            }
            else
            {
            ...
            *(crcLow) += dataElement;
            ...
            if (*(crcLow) < crcLowPrevious)
            {
                *(crcHigh) = *(crcHigh) + 1;
            }

            *(crcLow) = *(crcLow) ^ (crc32Table[*(crcLow) & 0xFF]);
            *(crcHigh) = *(crcHigh) ^ (crc32Table[*(crcHigh) & 0xFF]);

            crcLowPrevious = *(crcLow);
        }
    }
}
/*
...
{
    return (block->length / CANFTP_FILEBLOCK_FRAMESIZE) + ((block->length % CANFTP_FILEBLOCK_FRAMESIZE) > 0 ? 1 : 0);
}
/*
